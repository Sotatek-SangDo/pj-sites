...
add_filter( 'the_content', 'better_format_chat_content' );

if ( ! function_exists( 'better_format_chat_content' ) ) {
	/**
	 ...
	 */
	function better_format_chat_content( $content ) {

		global $_better_post_format_chat_ids;
		...
		$_better_post_format_chat_ids = array();

		$separator = apply_filters( 'better-template/chat-format/separator', ':' );
		...
		foreach ( $chat_rows as $chat_row ) {
			...
			if ( strpos( $chat_row, $separator ) ) {
				...
				$speaker_id = better_format_chat_row_id( $chat_author );
		 ...
		 *
		 */
		$collected_chat = apply_filters( 'better-template/chat-format/collected-chat', $collected_chat );
		...
		foreach ( $collected_chat as $chat_row ) {
			...
			if ( ! empty( $chat_row['speaker_name'] ) ) {
				$chat_output .= '<span class="user-name">' . $chat_row['speaker_name'] . '</span>';
			}
		 ...
		 *
		 */
		return apply_filters( 'better-template/chat-format/content', $chat_output );

	} // better_format_chat_content
} // if


if ( ! function_exists( 'better_format_chat_row_id' ) ) {
	/**
	 ...
	 */
	function better_format_chat_row_id( $chat_author ) {

		global $_better_post_format_chat_ids;

		$chat_author = strtolower( strip_tags( trim( $chat_author ) ) );

		if ( ! array_search( $chat_author, $_better_post_format_chat_ids ) ) {
			$_better_post_format_chat_ids[ count( $_better_post_format_chat_ids ) + 1 ] = $chat_author;
		}

		return absint( array_search( $chat_author, $_better_post_format_chat_ids ) );
	} // better_format_chat_row_id
} // if
