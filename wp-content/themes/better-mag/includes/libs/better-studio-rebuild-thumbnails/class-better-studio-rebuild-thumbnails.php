...
/**
 * Better Rebuild Thumbnails
 *
 * @package  Better Rebuild Thumbnails
 * @author   BetterStudio <info@betterstudio.com>
 * @version  1.0.2
 * @access   public
 * @see      http://www.betterstudio.com
 */
class Better_Studio_Rebuild_Thumbnails extends BF_Admin_Page{
    ...
    function __construct( $args = array() ){
        ...
        $args['id']     = 'better-studio-rebuild-thumbnails';
    ...
    function add_menu(){
        ...
        BF()->admin_menus()->add_menupage( array(
                ...
                'name' 				  =>    __( 'Rebuild Thumbnails', 'better-studio' ),
                'parent' 			  =>    'better-studio',
                'page_title'		  =>    __( 'Rebuild Thumbnails', 'better-studio' ),
                'menu_title'		  =>    __( 'Rebuild Thumbnails', 'better-studio' ),
                'position'  		  =>    50.02,
     ...
     */
    function get_title(){
        return __( 'Better Rebuild Thumbnails', 'better-studio' );
    }
     ...
     */
    function get_desc(){
        return '<p>' . __( 'Rebuild all thumbnails at once without script timeouts on your server.', 'better-studio' ) . '</p>';
    }
    ...
    function get_body(){
        ...
        <div class="thumbnails-rebuild-wrapper bf-clearfix">
            ...
            <div class="pre-desc">
                <p><?php _e( 'Hit following button to rebuild all thumbnails', 'better-studio' ); ?></p>

                <label>
                    <input type="checkbox" id="only_featured" name="only_featured" checked="checked"/>
                    <?php _e('Only rebuild featured images', 'better-studio'); ?>
                </label>
            </div>

            <div class="better-rebuild-all-thumbnails bf-button bf-main-button large-2x" id="better-rebuild-thumbnails">
                <span class="text-1"><i class="fa fa-refresh"></i> <?php _e( 'Rebuild All Thumbnails', 'better-studio' ) ?></span>
            </div>

            <div class="rebuild-log-container">
                <a class="show-rebuild-log"><?php _e( 'Show Rebuild Log', 'better-studio' ); ?></a>
    ...
    function admin_enqueue_scripts(){
        ...
        wp_enqueue_style( 'better-rebuild-thumbnails', $this->get_dir_uri() . 'assets/css/better-rebuild-thumbnails.css', array(), $this->get_version() );

        wp_enqueue_script( 'better-rebuild-thumbnails', $this->get_dir_uri() . 'assets/js/better-rebuild-thumbnails.js', array(), $this->get_version() );
        ...
        wp_localize_script(
            ...
            apply_filters(
                ...
                array(
                    ...
                    'text_confirm'          =>  __( "Are you sure do you want rebuild all thumbnails?", "better-studio" ),
                    'text_loading'          =>  '<div class="text-1">' .  __( 'Loading...', 'better-studio' ) . '</div>',
                    ...
                    'text_done'             =>  '<div class="text-1"><i class="fa fa-check"></i> ' .  __( 'Done', 'better-studio' ) . '</div>',
                    'text_rebuilding_state' =>  __( 'Rebuilding %number% of %all%', 'better-studio' ),
                    'text_no_image'         =>  '<i class="fa fa-exclamation"></i>' . __( 'No any image found.', 'better-studio' ),
                )
            )
        );
    ...
    function callback_rebuild_image() {
        ...
        if( false !== $attachment_url && @file_exists( $attachment_url ) ){
            ...
            $result['message'] = '<li class="success"><strong>' . __( 'Completed', 'better-studio' ) . '</strong>: <a target="_blank" href="' . wp_get_attachment_url( $attachment_id ) . '">' . $attachment_title . '</a></li>';

        }else{
            $result['status'] = 'error';
            $result['message'] = __( 'File not found', 'better-studio' );
            $result['message'] = '<li class="error"><strong>' . __( 'File not found', 'better-studio' ) . '</strong>: <a target="_blank" href="' . wp_get_attachment_url( $attachment_id ) . '">' . $attachment_title . '</a></li>';
