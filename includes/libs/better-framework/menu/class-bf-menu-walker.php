...
class BF_Menu_Walker extends Walker_Nav_Menu {
	...
	/**
	 * ID of current active mega menu ID
	 *
	 * @var array
	 */
	public $current_mega_menu_id;
		...
		if ( $this->in_mega_menu ) {
			...
			if ( $depth == 0 ) {
				...
				$mega_menu = apply_filters( 'better-framework/menu/mega/end_lvl', array(
						'depth'        => $depth,
						'this'         => &$this,
						'sub-menu'     => &$this->last_lvl,
						'current-item' => &$this->current_item,
						'output'       => '',
					)
				);
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! $depth ) {
			...
			foreach ( self::$mega_menu_field_ids as $mega_id => $mega_value ) {
				if ( property_exists( $item, $mega_id ) && ! empty( $item->{$mega_id} ) && $item->{$mega_id} != $mega_value['default'] && $depth <= intval( $mega_value['depth'] ) ) {
	...
	function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		if ( $this->in_mega_menu && ! $this->mega_menu_printed && $depth <= self::$mega_menu_field_ids[ $this->current_mega_menu_id ]['depth'] ) {
			...
			// todo: sub menu in filter have some extra closing tag in first of data, it should be fixed if we decide to show this on mega menu
			$mega_menu = apply_filters( 'better-framework/menu/mega/end_lvl', array(
					'depth'        => $depth,
					'this'         => &$this,
					'sub-menu'     => &$this->last_lvl,
