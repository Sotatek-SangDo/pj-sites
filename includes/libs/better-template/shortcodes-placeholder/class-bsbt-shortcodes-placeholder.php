...
BSBT_Shortcodes_Placeholder::Run();

class BSBT_Shortcodes_Placeholder {
	 ...
	 *
	 * @return \BSBT_Shortcodes_Placeholder
	 */
	...
	function load_shortcodes_config( $force = FALSE ) {
		if ( $force || ! $this->_configs_loaded ) {
			$this->shortcodes_config = apply_filters( 'better-template/shortcodes-placeholder/config', $this->shortcodes_config );
		}
	}
	 ...
	 */
	function register_shortcode( $shortcode = array() ) {
	...
	function show_placeholder( $shortcode = '', $atts = array(), $content ) {
		...
		$shortcode = wp_parse_args( $shortcode, array(
			...
			'notice_long'      => sprintf( __( 'The <b>%s</b> shortcode is not registered!', 'better-studio' ), $shortcode['id'] ),
			'notice_small'     => '',
			'notice_type'      => 'long',
		) );
		...
		echo '<p class="bsbt-shortcode-placeholder-p">', $raw_shortcode, '</p>';

		?>
		<div class="bsbt-shortcode-placeholder type-<?php echo $shortcode['notice_type']; ?>">
			<?php

			if ( $shortcode['notice_type'] == 'small' && ! empty( $shortcode['notice_small'] ) ) {
				echo $shortcode['notice_small'];
			} else {
				echo $shortcode['notice_long'];
			}
